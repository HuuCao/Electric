...
export default function TinhDienTro({navigation}) {
  ...
  const [isName2, setIsName2] = useState(LG.dongDien);
  const [isKiHieu2, setIsKiHieu2] = useState('I');
  const [isDonVi2, setIsDonVi2] = useState('A');
  const [showCos, setShowCos] = useState(false);
  const [isNameKq, setIsNameKq] = useState(LG.dienAp);
  const [isKiHieuKq, setIsKiHieuKq] = useState('U');
  ...
  const onTinhToan = function () {
    ...
    if (slType === 1 && slInput === 1) {
      console.log('Dòng điện - Điện áp đầu vào');
      setIsResults(isI / isP);
    } else if (slType === 1 && slInput === 3) {
      console.log('Dòng điện - Công suất đầu vào');
      setIsResults(isI / isP / isI);
    } else if (slType === 1 && slInput === 5) {
      console.log('Điện áp-công suất tác dụng  đầu vào');
      setIsResults((isI * isI) / isP);
    } else if (slType === 2 && slInput === 1) {
      console.log(LG.dienApDongDienDauVao);
      setIsResults((isI * isCos) / isP);
    } else if (slType === 2 && slInput === 2) {
      console.log(LG.congSuatTacDungDongDienDauVao);
      setIsResults(isI / isP / isP);
    } else if (slType === 2 && slInput === 3) {
      console.log(LG.congSuatDienApDauVao);
      setIsResults((isP * isP * isCos * isCos) / isI);
    } else if (slType === 2 && slInput === 4) {
      console.log(LG.congSuatBieuKienDongDienDauVao);
      setIsResults((isP * isCos) / isI / isI);
    } else if (slType === 2 && slInput === 5) {
      console.log(LG.congSuatBieuKienDienApDauVao);
      setIsResults((isI * isI * isCos) / isP);
    } else if (slType === 2 && slInput === 6) {
      console.log(LG.congSuatPhanKhangDongDienDauVao);
      setIsResults((isP * isCos) / isI / isI / Math.sqrt(1 - isCos * isCos));
    } else if (slType === 2 && slInput === 7) {
      console.log(LG.congSuatPhanKhangDienApDauVao);
      setIsResults((isI * isI * isCos * Math.sqrt(1 - isCos * isCos)) / isP);
    } else if (slType === 2 && slInput === 8) {
      console.log(LG.tongTroDauVao);
      ...
      console.log(LG.congSuatTacDungDongDienDauVao);
      setIsResults(isI / isP / isP / Math.sqrt(3));
    } else if (slType === 4 && slInput === 3) {
      console.log(LG.congSuatDienApDauVao);
      setIsResults((Math.sqrt(3) * isP * isP * isCos * isCos) / isI);
    } else if (slType === 4 && slInput === 4) {
      console.log(LG.congSuatBieuKienDongDienDauVao);
      setIsResults((isP * isCos) / isI / isI / Math.sqrt(3));
    } else if (slType === 4 && slInput === 5) {
      console.log(LG.congSuatBieuKienDienApDauVao);
      setIsResults((Math.sqrt(3) * isI * isI * isCos) / isP);
    } else if (slType === 4 && slInput === 6) {
      console.log(LG.congSuatPhanKhangDongDienDauVao);
      setIsResults(
        (isP * isCos) / isI / isI / Math.sqrt(1 - isCos * isCos) / Math.sqrt(3),
      );
    } else if (slType === 4 && slInput === 7) {
      console.log(LG.congSuatPhanKhangDienApDauVao);
      setIsResults(
        (Math.sqrt(3) * isI * isI * isCos * Math.sqrt(1 - isCos * isCos)) / isP,
      );
  ...
  const onConvertType = function (e) {
    if (e === 1) {
      //Điện áp 1 chiều
      setInputSelect([
        {label: LG.dongDienDienApDauVao, value: 1},
        {label: LG.dongDienCongSuatDauVao, value: 3},
        {label: LG.dienApCongSuatDauVao, value: 5},
      ]);
    } else if (e === 2) {
      //1 pha xoay chiều
      setInputSelect([
        {label: LG.dienApDongDienDauVao, value: 1},
        {label: LG.congSuatTacDungDongDienDauVao, value: 2},
        {label: LG.congSuatDienApDauVao, value: 3},
        {label: LG.congSuatBieuKienDongDienDauVao, value: 4},
        {label: LG.congSuatBieuKienDienApDauVao, value: 5},
        {label: LG.congSuatPhanKhangDongDienDauVao, value: 6},
        {label: LG.congSuatPhanKhangDienApDauVao, value: 7},
        {label: LG.tongTroDauVao, value: 8},
      ]);
    } else if (e === 3) {
      //2 pha xoay chiều
      setInputSelect([
  ...
  const onConvertInput = function (e) {
    ...
    if (slType === 1 && e === 1) {
      ...
      setIsDonVi2('V');
      setShowCos(false);
      setIsNameKq(LG.dienTro);
      setIsKiHieuKq('R');
    } else if (slType === 1 && e === 3) {
      console.log('Dòng điện - Công suất đầu vào');
      setIsName1(LG.congSuat);
      setIsKiHieu1('P');
      setIsDonVi1('W');
  ...
  return (
    <SafeAreaView style={styles.viewSafe}>
      <TouchableOpacity
        ...
        />
        <Text style={styles.txtBack}>{LG.tinhDienTro}</Text>
      </TouchableOpacity>
