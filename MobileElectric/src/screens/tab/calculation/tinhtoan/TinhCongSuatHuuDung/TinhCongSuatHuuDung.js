...
export default function TinhToanCongSuatHuuDung({navigation}) {
  ...
  const [isName2, setIsName2] = useState(LG.dienApDay);
  const [isKiHieu2, setIsKiHieu2] = useState('U');
  const [isDonVi2, setIsDonVi2] = useState('V');
  const [showCos, setShowCos] = useState(false);
  const [isNameKq, setIsNameKq] = useState(LG.congSuat);
  const [isKiHieuKq, setIsKiHieuKq] = useState('P');
  ...
  const onResults = function () {
    if (
      ...
      if (+isP === 0 || isP === '0,0' || checkInput.test(isP) === false) {
        ...
        setIsStatus('Dòng điện ' + isI + LG.khongHopLe);
        setShowPopup(!showPopup);
        setShowResults(false);
          ...
          if (isCos.length !== 0) {
            if (showCos === true) {
              if (
                ...
                setIsStatus('Hệ số công suất ' + isCos + LG.khongHopLe);
                setShowPopup(!showPopup);
                setShowResults(false);
              } else {
                onTinhToan();
  ...
  const onTinhToan = function () {
    ...
    if (slType === 1 && slInput === 1) {
      ...
      console.log('Dòng điện / điện trở đầu vào');
      setIsResults(isP * isI * isI);
    } else if (slType === 1 && slInput === 5) {
      console.log('1-5');
      setIsResults((isI * isI) / isP);
    } else if (slType === 2 && slInput === 1) {
      console.log(LG.dongDienDienApDauVao);
      setIsResults(isP * isI * isCos);
    } else if (slType === 2 && slInput === 2) {
      console.log(LG.tongTroDongDienDauVao);
      setIsResults(isI * isP * isP * isCos);
    } else if (slType === 2 && slInput === 3) {
      console.log(LG.dienTroDongDienDauVao);
      setIsResults(isI * isP * isP);
    } else if (slType === 2 && slInput === 4) {
      console.log(LG.tongTroDienApDauVao);
      setIsResults((isP * isP * isCos) / isI);
    } else if (slType === 2 && slInput === 5) {
      console.log(LG.dienTroDienApDauVao);
      setIsResults((isP * isP * isCos * isCos) / isI);
    } else if (slType === 2 && slInput === 6) {
      console.log(LG.congSuatBieuKien);
      setIsResults(isP * isI);
    } else if (slType === 2 && slInput === 7) {
      console.log(LG.congSuatPhanKhang);
      setIsResults(isP / (1 / isI / isI - 1));
    } else if (slType === 2 && slInput === 8) {
      console.log(LG.congSuatPhanKhangCongSuatBieuKien);
      setIsResults(Math.sqrt(isI * isI - isP * isP));
    } else if (slType === 3 && slInput === 1) {
      console.log(LG.dongDienDienApDauVao);
      setIsResults(isP * isI * isCos);
    } else if (slType === 3 && slInput === 2) {
      console.log(LG.tongTroDongDienDauVao);
      setIsResults(isI * isP * isCos);
    } else if (slType === 3 && slInput === 3) {
      console.log(LG.dienTroDongDienDauVao);
      setIsResults(isI * isP * isP);
    } else if (slType === 3 && slInput === 4) {
      console.log(LG.tongTroDienApDauVao);
      setIsResults((isP * isP * isI) / isCos);
    } else if (slType === 3 && slInput === 5) {
      console.log(LG.dienTroDienApDauVao);
      setIsResults((isP * isP * isI * isI) / isCos);
  ...
  const onConvertType = function (e) {
    if (e === 1) {
      //Điện áp 1 chiều
      setInputSelect([
        {label: 'Dòng điện/Điện áp đầu vào', value: 1},
        {label: LG.congSuatDienTroDauVao, value: 3},
        {label: 'Điện áp/Điện trở đầu vào', value: 5},
      ]);
    } else if (e === 2) {
      //1 pha xoay chiều
      setInputSelect([
        {label: LG.dongDienDienApDauVao, value: 1},
        {label: LG.tongTroDongDienDauVao, value: 2},
        {label: LG.dienTroDongDienDauVao, value: 3},
        {label: LG.tongTroDienApDauVao, value: 4},
        {label: LG.dienTroDienApDauVao, value: 5},
        {label: LG.congSuatBieuKien, value: 6},
        {label: LG.congSuatPhanKhang, value: 7},
        {label: LG.congSuatPhanKhangCongSuatBieuKien, value: 8},
      ]);
    } else if (e === 3) {
      //2 pha xoay chiều
      setInputSelect([
  ...
  return (
    <SafeAreaView style={styles.viewSafe}>
      <TouchableOpacity
        ...
        />
        <Text style={styles.txtBack}>{LG.tinhCongSuatHuuDung}</Text>
      </TouchableOpacity>
            ...
            {showResults === true && (
              <View style={styles.viewResult}>
                <View style={styles.viewItem}>
                  ...
                  <View style={styles.viewUnit}>
                    <Text style={styles.txtTitle1}>{'W'}</Text>
                  </View>
                </View>
              </View>
            )}
